...
import wallet.management.wallet.Model.Balance;
...
public class BalanceRepository implements BasicRepository<Balance>{
    ...
    public Balance save(Balance toSave, String idAccount) throws SQLException {
        ...
        values.put(Query.ID_LABEL, new Pair(toSave.getId(), true));
        values.put(AMOUNT_LABEL, new Pair(toSave.getAmount().toString(), false));
        values.put(CREATION_DATETIME, new Pair(dateTime != null ? dateTime.toString() : null, true));
        values.put(ACCOUNT_LABEL, new Pair(idAccount, true));
