package wallet.management.wallet.Repository;

import lombok.AllArgsConstructor;
import wallet.management.wallet.Model.*;
import java.sql.*;
...
import static wallet.management.wallet.Repository.Query.ID_LABEL;
...
public class AccountRepository implements BasicRepository<Account> {
    ...
    private static Account createInstance(ResultSet resultSet) throws SQLException {
        String accountId = resultSet.getString(ID_LABEL);
        ...
        Map<String, Pair> currencyFilter = Map.of(ID_LABEL, new Pair(resultSet.getString(CURRENCY_LABEL), true));
    ...
    private Map<String, Pair> createFilter(String key, String value) {
        return Map.of(key, new Pair(value, true));
    }
    @Override
    public List<Account> findAll(Map<String, Pair> filters) throws SQLException {
        List<Account> results = new ArrayList<>();
        try (ResultSet resultSet = Query.selectAll(TABLE_NAME, filters, null)) {
    ...
    public Account save(Account toSave, String meta) throws SQLException {
        Map<String, Pair> values = Map.of(
                ID_LABEL, new Pair(toSave.getId(), true),
    ...
    public Account doTransaction(Transaction transaction, String accountId) throws SQLException {
        if (accountId != null) {
            Map<String, Pair> accountFilter = Map.of(ID_LABEL, new Pair(accountId, true));
    ...
    private void updateBalance(Account target, Transaction transaction) throws SQLException {
        BigDecimal amountToAdd = getTransactionAmountToAdd(transaction);
        BigDecimal newBalance = target.getBalance().getAmount().add(amountToAdd);

        balanceRepository.save(new Balance(null, newBalance, null), target.getId());
    }
    private BigDecimal getTransactionAmountToAdd(Transaction transaction) {
        return transaction.getType() == TransactionType.CREDIT ? transaction.getAmount() : transaction.getAmount().negate();
    }
